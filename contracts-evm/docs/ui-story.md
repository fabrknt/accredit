...
- Sub: *A regulated stablecoin (cUSDC) on HashKey Chain. KYC + AI-AML compliance, enforced on-chain — live.*
- Badge: HashKey Testnet · chainId 133

## Guided steps (numbered strip; click → scroll to section + highlight)
1. **Onboard verified users** — Only KYC-verified, AML-screened wallets can hold or receive cUSDC. → `sec-identity`
...
5. **Control & adoption** — Freeze/recover compromised funds; wrap existing USDC into compliance 1:1. → `sec-agent`

## Section "why" subtitles
- Identity & Risk → *Who may hold cUSDC — and their live AML risk.*
...
- Compliant Wrapping → *Pull existing USDC liquidity into the compliance layer, 1:1.*
